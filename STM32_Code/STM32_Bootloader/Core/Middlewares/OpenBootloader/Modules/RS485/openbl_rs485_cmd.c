/**
  ******************************************************************************
  * @file    openbl_rs485_cmd.c
  * @author  MCD Application Team    modified by Philip Kiehnle
  * @brief   Contains RS485 protocol commands
  ******************************************************************************
  * @attention
  *
...
/* Private typedef -----------------------------------------------------------*/
/* Private define ------------------------------------------------------------*/
#define OPENBL_RS485_COMMANDS_NB          11U       /* Number of supported commands */

#define RS485_RAM_BUFFER_SIZE             1156U     /* Size of RS485 buffer used to store received data from the host */
...
static uint8_t RS485_RAM_Buf[RS485_RAM_BUFFER_SIZE];

/* Private function prototypes -----------------------------------------------*/
static void OPENBL_RS485_GetCommand(void);
static void OPENBL_RS485_GetVersion(void);
static void OPENBL_RS485_GetID(void);
static void OPENBL_RS485_ReadMemory(void);
static void OPENBL_RS485_WriteMemory(void);
static void OPENBL_RS485_Go(void);
static void OPENBL_RS485_ReadoutProtect(void);
static void OPENBL_RS485_ReadoutUnprotect(void);
static void OPENBL_RS485_EraseMemory(void);
static void OPENBL_RS485_WriteProtect(void);
static void OPENBL_RS485_WriteUnprotect(void);


static uint8_t OPENBL_RS485_GetAddress(uint32_t *Address);


/* Exported variables --------------------------------------------------------*/
OPENBL_CommandsTypeDef OPENBL_RS485_Commands =
{
  OPENBL_RS485_GetCommand,
  OPENBL_RS485_GetVersion,
  OPENBL_RS485_GetID,
  OPENBL_RS485_ReadMemory,
  OPENBL_RS485_WriteMemory,
  OPENBL_RS485_Go,
  OPENBL_RS485_ReadoutProtect,
  OPENBL_RS485_ReadoutUnprotect,
  OPENBL_RS485_EraseMemory,
  OPENBL_RS485_WriteProtect,
  OPENBL_RS485_WriteUnprotect,
  NULL,
  NULL,
  NULL,
  NULL,
...
/* Exported functions---------------------------------------------------------*/
/**
  * @brief  This function is used to get a pointer to the structure that contains the available RS485 commands.
  * @return Returns a pointer to the OPENBL_RS485_Commands struct.
  */
OPENBL_CommandsTypeDef *OPENBL_RS485_GetCommandsList(void)
{
  return (&OPENBL_RS485_Commands);
}

/* Private functions ---------------------------------------------------------*/

/**
  * @brief  This function is used to get the list of the available RS485 commands
  * @retval None.
  */
static void OPENBL_RS485_GetCommand(void)
{
  uint32_t counter;
  const uint8_t a_OPENBL_RS485_CommandsList[OPENBL_RS485_COMMANDS_NB] =
  {
    CMD_GET_COMMAND,
    CMD_GET_VERSION,
    CMD_GET_ID,
  ...
  /* Send the number of commands supported by the RS485 protocol */
  OPENBL_RS485_SendByte(OPENBL_RS485_COMMANDS_NB);

  /* Send RS485 protocol version */
  OPENBL_RS485_SendByte(OPENBL_RS485_VERSION);

  /* Send the list of supported commands */
  for (counter = 0U; counter < OPENBL_RS485_COMMANDS_NB; counter++)
  {
    OPENBL_RS485_SendByte(a_OPENBL_RS485_CommandsList[counter]);
  }
...
/**
  * @brief  This function is used to get the RS485 protocol version.
  * @retval None.
  */
static void OPENBL_RS485_GetVersion(void)
{
...
static void OPENBL_RS485_GetID(void)
{
  ...
  OPENBL_RS485_SendByte(0x01);

  /* Send the device ID starting by the MSB byte then the LSB byte */
  OPENBL_RS485_SendByte(DEVICE_ID_MSB);
  OPENBL_RS485_SendByte(DEVICE_ID_LSB);
...
static void OPENBL_RS485_ReadMemory(void)
{
  ...
  {
    OPENBL_RS485_SendByte(NACK_BYTE);
  }
  else
  {
    OPENBL_RS485_SendByte(ACK_BYTE);
    ...
    if (OPENBL_RS485_GetAddress(&address) == NACK_BYTE)
    {
      OPENBL_RS485_SendByte(NACK_BYTE);
    }
    else
    {
      ...
      data = OPENBL_RS485_ReadByte();
      xor  = ~data;

      /* Check data integrity */
      if (OPENBL_RS485_ReadByte() != xor)
      {
        OPENBL_RS485_SendByte(NACK_BYTE);
      }
      else
      {
        ...
        {
          OPENBL_RS485_SendByte(OPENBL_MEM_Read(address, memory_index));
          address++;
        }
      }
    }
...
static void OPENBL_RS485_WriteMemory(void)
{
  ...
  else
  {
    ...
    else
    {
      ...
      ramaddress = (uint8_t *)RS485_RAM_Buf;
      ...
      if (OPENBL_RS485_ReadByte() != tmpXOR)
      {
        OPENBL_RS485_SendByte(NACK_BYTE);
      }
      else
      {
        ...
        OPENBL_MEM_Write(address, (uint8_t *)RS485_RAM_Buf, codesize);
...
static void OPENBL_RS485_EraseMemory(void)
{
  ...
  else
  {
    ...
    data = OPENBL_RS485_ReadByte();
    data = (uint16_t)(data << 8) | OPENBL_RS485_ReadByte();
    ...
    {
      /* Check data integrity */
      if (OPENBL_RS485_ReadByte() != (uint8_t) xor)
      {
        status = NACK_BYTE;
      }
      else
      {
        if ((data == 0xFFFFU) || (data == 0xFFFEU) || (data == 0xFFFDU))
        {
          ...
          error_value = OPENBL_MEM_MassErase(FLASH_START_ADDRESS, (uint8_t *) RS485_RAM_Buf, RS485_RAM_BUFFER_SIZE);
    ...
    else
    {
      ...
      *ramaddress = (uint8_t)((numpage & 0xFF00U) >> 8);
      ...
      for (counter = numpage; counter != 0U ; counter--)
      {
        ...
        data |= (uint8_t)(OPENBL_RS485_ReadByte() & 0x00FFU);
        xor  ^= ((uint32_t)data & 0x00FFU);

        /* Only store data that fit in the buffer length */
        if (counter < (RS485_RAM_BUFFER_SIZE / 2U))
        {
        ...
        {
          /* Continue to read data from RS485 */
        }
      ...
      {
        error_value = OPENBL_MEM_Erase(FLASH_START_ADDRESS, (uint8_t *) RS485_RAM_Buf, RS485_RAM_BUFFER_SIZE);
...
static uint8_t OPENBL_RS485_GetAddress(uint32_t *Address)
{
  ...
  data[3] = OPENBL_RS485_ReadByte();
  data[2] = OPENBL_RS485_ReadByte();
  data[1] = OPENBL_RS485_ReadByte();
  data[0] = OPENBL_RS485_ReadByte();
